...
def _seat_is_occupied(model, x, y):
    if x < 0 or y < 0 or x >= room_height or y >= room_width:
        return False
    return model[x][y] in ('#', )


def _adjacent(model, x, y):
    adjacencies = []
    # top left
    adjacencies.append(_seat_is_occupied(model, x-1, y-1))
    # top
    adjacencies.append(_seat_is_occupied(model, x-1, y))
    # top right
    adjacencies.append(_seat_is_occupied(model, x-1, y+1))
    # left
    adjacencies.append(_seat_is_occupied(model, x, y-1))
    # right
    adjacencies.append(_seat_is_occupied(model, x, y+1))
    # bottom left
    adjacencies.append(_seat_is_occupied(model, x+1, y-1))
...
def _recalculate_seat(model, x, y):
    adjacencies = sum(int(x) for x in _adjacent(model, x, y))
    if adjacencies == 0:  # No neighbours -> sit down
        return '#'
    if adjacencies >= 4:  # 4+ neighbours -> get up
        return 'L'
